"""Ejecuta EXP-025: brake_transition solo cuando estocástico previo está en zona extrema."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_025")
...
def evaluate(feats: pd.DataFrame, pair: str) -> pd.DataFrame:
    df = feats.copy()
    df = df.dropna(subset=["k", "d", "impulse_net", "brake_transition"])
    mask = df["brake_transition"].values & ((df["k"].values < 0.2) | (df["k"].values > 0.8))
    n = len(df)
    valid = []
...
def main() -> None:
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-transition-stoch-extreme",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-025 did not produce a gate decision")
