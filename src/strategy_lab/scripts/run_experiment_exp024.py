"""Ejecuta EXP-024: consistencia estricta 3/3 en horizontes 1-3."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_024")
...
BASELINE_WR = 0.371
HORIZONS = [1, 2, 3]
CONSISTENCY_HORIZONS = [1, 3]
MIN_CONSISTENT_WINS = 2
OUTCOME_HORIZON = 2


def evaluate_multi_horizon(feats: pd.DataFrame, pair: str) -> pd.DataFrame:
    mask = feats["brake_transition"].values
    n = len(feats)
    valid = []
    for idx in np.where(mask)[0]:
        if idx + max(CONSISTENCY_HORIZONS + [OUTCOME_HORIZON]) + 1 >= n:
            continue
        wins = 0
        entry_close = feats.iloc[idx + 1]["close"]
        direction = 1 if feats.iloc[idx]["impulse_net"] < 0 else -1
        for h in CONSISTENCY_HORIZONS:
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        events = evaluate_multi_horizon(feats, pair)
        if len(events) > 0:
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-multi-horizon-consistency-1-3",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-024 did not produce a gate decision")
