"""Ejecuta EXP-011: brake_transition alta calidad + horizonte 2 velas."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_011")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        indices = np.where((mask) & (body_n >= 2.0) & (brake_ratio >= 2.0))[0]
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-quality-horizon2",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-011 did not produce a gate decision")
