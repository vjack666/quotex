"""Ejecuta EXP-007: solo martillo M15 sin freno ni cruce."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_007")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        for idx in indices:
            ...
            direction = 1 if feats.iloc[idx]["close"] < feats.iloc[idx]["open"] else -1
            if direction == 1:
            ...
            pair_events.append({
                ...
                "timeframe": "M15",
                "hammer_idx": int(feats.iloc[idx]["idx"]),
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-martillo-sin-freno",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-007 did not produce a gate decision")
