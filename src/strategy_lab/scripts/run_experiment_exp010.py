"""Ejecuta EXP-010: brake_transition con filtros de calidad del freno."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_010")
PAIRS = ["EURUSD", "GBPUSD", "AUDUSD", "NZDUSD", "USDCAD", "USDCHF", "USDJPY"]


def evaluate_thresholds(df: pd.DataFrame, feats: pd.DataFrame, body_n_thresholds: list[float], brake_ratio_thresholds: list[float]) -> pd.DataFrame:
    results = []
    for body_n_th in body_n_thresholds:
        for brake_ratio_th in brake_ratio_thresholds:
            mask = feats["brake_transition"].values
            body_n = feats["body_n_brake"].values
            ...
            if len(pair_events) == 0:
                continue
            events = pd.DataFrame(pair_events)
            win_rate = float(events["win"].mean())
            ev = float(events["expected_value"].sum())
            profits = events["profit"].values
            pf = float(np.sum(profits[profits > 0]) / abs(np.sum(profits[profits < 0]))) if np.sum(profits < 0) != 0 else float("inf")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        indices = np.where((mask) & (body_n >= 0.8) & (brake_ratio >= 1.5))[0]
        ...
        for idx in indices:
            ...
            pair_events.append({
                ...
                "body_n_brake": float(feats.iloc[idx]["body_n"]),
                "brake_ratio": float(feats.iloc[idx]["brake_ratio"]),
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-transition-calidad",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-010 did not produce a gate decision")
