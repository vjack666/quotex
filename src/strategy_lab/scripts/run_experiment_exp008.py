"""Ejecuta EXP-008: martillo M15 solo dentro de ventana post-cruce."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_008")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        for i in range(n):
            ...
            outcome = feats.iloc[outcome_idx]
            direction = 1 if feats.iloc[i]["k"] > feats.iloc[i]["d"] else -1
            if direction == 1:
            ...
            pair_events.append({
                ...
                "timeframe": "M15",
                "cross_idx": int(feats.iloc[i]["idx"]),
                "hammer_idx": int(feats.iloc[hammer_idx]["idx"]),
                "minutes_cross_to_hammer": (hammer_idx - i) * 15,
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-martillo-post-cruce-contexto",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-008 did not produce a gate decision")
