"""Ejecuta EXP-014: brake_transition (1.5, 1.5) + horizonte 3 velas."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_014")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        indices = np.where((mask) & (body_n >= 1.5) & (brake_ratio >= 1.5))[0]
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-quality-1.5-1.5-horizon3",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-014 did not produce a gate decision")
