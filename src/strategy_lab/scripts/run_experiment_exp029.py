"""Ejecuta EXP-029: consistencia 3/5 + filtro rvol >= 1.5."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_029")
...
def evaluate(feats: pd.DataFrame, pair: str) -> pd.DataFrame:
    df = feats.copy()
    df = df.dropna(subset=["rvol", "impulse_net", "brake_transition"])
    mask = df["brake_transition"].values & (df["rvol"].values >= RVOL_TH)
    n = len(df)
    valid = []
...
def main() -> None:
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-transition-consistency-rvol",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-029 did not produce a gate decision")
