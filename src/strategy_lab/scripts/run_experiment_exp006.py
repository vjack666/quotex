"""Ejecuta EXP-006: solo cruce del estocástico en zona extrema, sin freno ni martillo."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_006")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        mask = feats["cruce_en_zona"].values
        ...
        for idx in indices:
            ...
            direction = 1 if feats.iloc[idx]["k"] > feats.iloc[idx]["d"] else -1
            if direction == 1:
            ...
            pair_events.append({
                ...
                "timeframe": "M15",
                "cross_idx": int(feats.iloc[idx]["idx"]),
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-cruce-en-zona-extrema-sin-freno",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-006 did not produce a gate decision")
