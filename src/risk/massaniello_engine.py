...
def _simulate_massaniello(settings: Settings, history: Iterable[str]) -> Simulation:
    ...
    for index, result in enumerate(results, start=1):
        ...
        rows.append(BetRow(number=index, result=result, stake=stake, massaniello_stake=stake, calculator_stake=calculator_stake, difference=difference, return_amount=return_amount, capital=capital, wins=wins_display, losses=losses_display, itm_percent=itm_percent, withdrawal=withdrawal, status=status))
    ...
    target_multiplier = _multiplier_table(settings.operations, settings.expected_itm, settings.profit)[0][0]
    ...
    return Simulation(settings=settings, rows=rows, current_capital=capital, next_stake=next_stake, next_massaniello_stake=next_stake, next_calculator_stake=next_calculator_stake, next_difference=next_difference, next_number=(len(rows) + 1 if not finished else None), wins=wins, losses=losses, withdrawals=withdrawals, status=status, finished=finished, target_capital=target_capital, minimum_net_profit=target_capital - settings.initial_balance, balance_objective=balance_objective, objective_entry=objective_entry)
...
def _simulate_objective_system(settings: Settings, history: Iterable[str]) -> Simulation:
    ...
    for index, result in enumerate(results, start=1):
        ...
        _, calculator_stake = calculate_balance_objective(settings, capital)
        stake = min(massaniello_stake, calculator_stake) if settings.system_mode == "hybrid" and massaniello_stake is not None else calculator_stake
        ...
        if result == "w":
            ...
            losses += 1
        status = "Ciclo objetivo" if result == "w" else f"Gale {losses}"
        rows.append(BetRow(number=index, result=result, stake=stake, massaniello_stake=massaniello_stake, calculator_stake=calculator_stake, difference=difference, return_amount=return_amount, capital=capital, wins=wins, losses=losses, itm_percent=wins / index if index else None, withdrawal=0.0, status=status))

    massaniello_stake = calculate_stake(settings, capital, wins, losses)
    _, calculator_stake = calculate_balance_objective(settings, capital)
    next_stake = min(massaniello_stake, calculator_stake) if settings.system_mode == "hybrid" and massaniello_stake is not None else calculator_stake
    next_difference = None if massaniello_stake is None else massaniello_stake - calculator_stake
    balance_objective, objective_entry = calculate_balance_objective(settings, capital)

    return Simulation(settings=settings, rows=rows, current_capital=capital, next_stake=next_stake, next_massaniello_stake=massaniello_stake, next_calculator_stake=calculator_stake, next_difference=next_difference, next_number=len(rows) + 1, wins=wins, losses=losses, withdrawals=0.0, status=_system_status(settings.system_mode, next_difference), finished=capital <= 0, target_capital=balance_objective, minimum_net_profit=max(0.0, balance_objective - capital), balance_objective=balance_objective, objective_entry=objective_entry)
...
def _system_status(system_mode: SystemMode, difference: float | None) -> str:
    ...
    if system_mode == "hybrid":
        return "Modo hibrido" if difference is None else "Hibrido: usa la menor entrada"
    return "Massaniello"


def _is_finished(settings: Settings, wins: int, losses: int, played: int) -> bool:
    return played >= settings.operations or wins >= settings.expected_itm or losses >= settings.operations - settings.expected_itm + 1
