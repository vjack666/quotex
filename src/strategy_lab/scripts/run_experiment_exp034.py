"""Ejecuta EXP-034: pipeline completo con condiciones confirmadas."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_034")
...
def evaluate_variant(feats: pd.DataFrame, pair: str) -> pd.DataFrame:
    df = feats.copy()
    df = df.dropna(subset=["brake_confirmed", "cross_clean_confirmed", "impulse_net", "close", "time"]).reset_index(drop=True)
    n = len(df)
    events = []
    for idx in range(n):
        if not bool(df.iat[idx, df.columns.get_loc("brake_confirmed")]):
            continue
        if not bool(df.iat[idx, df.columns.get_loc("cross_clean_confirmed")]):
            continue
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        events = evaluate_variant(feats, pair)
        all_events.append(events)
        print(f"{pair}: {len(events)} events")

    df_all = pd.concat(all_events, ignore_index=True)
    metrics = compute_metrics(df_all)
    print(f"pipeline confirmado: WR={metrics['win_rate']:.4f}, EV={metrics['expected_value']:.4f}, PF={metrics['profit_factor']:.4f}, n={metrics['events']}")
