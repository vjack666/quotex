"""Ejecuta EXP-027: consistencia 3/5 precio + consistencia 3/5 impulso."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_027")
...
BASELINE_WR = 0.371
HORIZONS = [1, 2, 3, 4, 5]
MIN_CONSISTENT_WINS = 3
OUTCOME_HORIZON = 2


def evaluate(feats: pd.DataFrame, pair: str) -> pd.DataFrame:
    df = feats.copy()
    df = df.dropna(subset=["impulse_net", "brake_transition", "close"])
    ...
    for idx in np.where(mask)[0]:
        ...
        impulse_wins = 0
        for h in HORIZONS:
            outcome = df.iloc[idx + 1 + h]["close"]
            if direction == 1:
                price_wins += 1 if entry_close < outcome else 0
            else:
                price_wins += 1 if entry_close > outcome else 0
            impulse_wins += 1 if (df.iloc[idx + 1 + h]["impulse_net"] < 0) == (direction == 1) else 0

        if price_wins < MIN_CONSISTENT_WINS or impulse_wins < MIN_CONSISTENT_WINS:
            continue

        outcome = df.iloc[idx + 1 + OUTCOME_HORIZON]["close"]
        if direction == 1:
...
def main() -> None:
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-transition-price-impulse-consistency",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-027 did not produce a gate decision")
