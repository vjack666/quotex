"""Orquestación de prefetch de datos para ciclos de escaneo."""
...
def filter_scan_assets(
    ...
    *,
    ...
    """Filtra activos elegibles para evaluación."""
...
async def prefetch_primary_candles(
    ...
    """Descarga velas 5m, 1m y 15m en paralelo con semáforo compartido."""
    if not symbols:
        return {}, {}, {}
    sem = ws_sem if ws_sem is not None else asyncio.Semaphore(max(1, int(concurrency)))
    tasks = []
    for sym in symbols:
        tasks.append(_fetch_with_optional_stagger(sem, client, sym, TF_5M, CANDLES_LOOKBACK, timeout_sec=CANDLE_FETCH_TIMEOUT_SEC, cache=cache))
        tasks.append(_fetch_with_optional_stagger(sem, client, sym, 60, 36, timeout_sec=CANDLE_FETCH_1M_TIMEOUT_SEC, cache=cache))
        tasks.append(_fetch_with_optional_stagger(sem, client, sym, TF_15M, 120, timeout_sec=CANDLE_FETCH_TIMEOUT_SEC, cache=cache, retries=1))
...
def _resolve_ob_candles(symbol: str, ob_candles: list[Candle], candles_5m_fallback: dict[str, list[Candle]]) -> tuple[list[Candle], str]:
    if len(ob_candles) < 6:
...
async def prefetch_strat_a_secondary(
    ...
    concurrency: int,
    ws_sem: "asyncio.Semaphore | None" = None,
) -> tuple[dict[str, list[Candle]], dict[str, list[Candle]], dict[str, str], dict[str, dict[str, list]]]:
    """Prefetch paralelo de velas OB (3m) y H1 para STRAT-A."""
    if not symbols:
        return {}, {}, {}, {}
    ...
    tasks = []
    for sym in symbols:
        tasks.append(_fetch_with_optional_stagger(sem, client, sym, ORDER_BLOCK_TF_SEC, ORDER_BLOCK_CANDLES, timeout_sec=CANDLE_FETCH_TIMEOUT_SEC, cache=cache, retries=1))
        tasks.append(_fetch_with_optional_stagger(sem, client, sym, H1_TF_SEC, H1_CANDLES_LOOKBACK, timeout_sec=H1_FETCH_TIMEOUT_SEC, cache=cache))
    results = await asyncio.gather(*tasks)
...
async def prefetch_historical_m15_initial(
    ...
    *,
    ...
    """Precarga histórico M15 al conectar."""
    if not symbols:
        return {}
    sem = ws_sem if ws_sem is not None else asyncio.Semaphore(max(1, int(concurrency)))
    tasks = [
        _fetch_with_optional_stagger(sem, client, sym, TF_15M, HISTORICAL_M15_COUNT, timeout_sec=CANDLE_FETCH_TIMEOUT_SEC, cache=cache, retries=2)
        for sym in symbols
    ]
    results = await asyncio.gather(*tasks)
    candles_15m: dict[str, list[Candle]] = {}
