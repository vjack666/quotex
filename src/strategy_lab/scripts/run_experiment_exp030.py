"""Ejecuta EXP-030: pipeline secuencial freno → cruce estocástico → liberación."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_030")
...
def evaluate(feats: pd.DataFrame, pair: str) -> pd.DataFrame:
    df = feats.copy()
    df = df.dropna(subset=["brake_transition", "cross_ago", "k", "d", "close", "impulse_net"])
    k = df["k"].values
    d = df["d"].values
    n = len(df)
    valid = []
    ...
    for idx in range(n):
        if waiting_for_cross:
            if df.iloc[idx]["cross_ago"] > MAX_CROSS_AGO:
                waiting_for_cross = False
                cross_dir = None
                continue
            if cross_dir == 1 and k[idx] <= 20 and k[idx - 1] <= d[idx - 1] and k[idx] > d[idx]:
                waiting_for_cross = False
            elif cross_dir == -1 and k[idx] >= 80 and k[idx - 1] >= d[idx - 1] and k[idx] < d[idx]:
                waiting_for_cross = False
            elif cross_dir == 1 and k[idx] > 20 and d[idx] > 20:
                released = True
                release_idx = idx
            elif cross_dir == -1 and k[idx] < 80 and d[idx] < 80:
                released = True
                release_idx = idx
        else:
            if df.iloc[idx]["brake_transition"]:
                waiting_for_cross = True
                cross_dir = 1 if df.iloc[idx]["impulse_net"] < 0 else -1
                released = False
                release_idx = None

        if released and release_idx is not None:
            if release_idx + 2 >= n:
                continue
            entry_close = df.iloc[release_idx + 1]["close"]
            outcome = df.iloc[release_idx + 2]["close"]
            if cross_dir == 1:
            ...
            valid.append({
                "asset": pair,
                "timestamp": df.iloc[release_idx]["time"],
                "win": win,
...
def main() -> None:
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-cross-release-pipeline",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-030 did not produce a gate decision")
