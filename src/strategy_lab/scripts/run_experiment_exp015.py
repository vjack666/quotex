"""Ejecuta EXP-015: brake_transition alineado con trend de corto plazo."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_015")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        trend = feats["trend"].values
        impulse_net = feats["impulse_net"].values
        
        indices = np.where((mask) & (trend * impulse_net > 0))[0]
        ...
        for idx in indices:
            ...
            direction = 1 if impulse_net[idx] < 0 else -1
            if direction == 1:
            ...
            pair_events.append({
                ...
                "body_n": float(feats.iloc[idx]["body_n"]),
    ...
    artifacts = run_experiment(
        ...
        events=events,
        hypothesis="H1-brake-trend-aligned",
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-015 did not produce a gate decision")
