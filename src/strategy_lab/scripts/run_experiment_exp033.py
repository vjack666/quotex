"""Ejecuta EXP-033: edge de cross_clean_confirmed aislado."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_033")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        events = evaluate_variant(feats, pair, "cross_clean_confirmed")
        all_events.append(events)
        print(f"{pair}: {len(events)} events")

    df_all = pd.concat(all_events, ignore_index=True)
    metrics = compute_metrics(df_all)
    print(f"cross_clean_confirmed: WR={metrics['win_rate']:.4f}, EV={metrics['expected_value']:.4f}, PF={metrics['profit_factor']:.4f}, n={metrics['events']}")
