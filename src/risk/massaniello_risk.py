...
class MassanielloRiskManager:
    ...
    def __init__(self, operations: Optional[int] = None, expected_wins: Optional[int] = None, session_max_min: Optional[int] = None) -> None:
        ...
        log.info("MassanielloRiskManager listo: %d ops / %d ITM / max %d min (motor calculadora)", self.operations, self.expected_wins, self.session_max_min)
    ...
    def _settings(self, payout_pct: int) -> Settings:
        ...
        return Settings(initial_balance=max(base, 0.01), operations=self.operations, expected_itm=self.expected_wins, profit=profit, mode="normal", system_mode="massaniello")
    ...
    def can_enter(self) -> bool:
        if self.current_balance is None or self.current_balance < MIN_ORDER_AMOUNT:
            return False
        return not (self.is_session_complete() or self.is_session_failed() or self.is_session_timeout() or self.is_session_exhausted())

    def next_stake(self, payout_pct: int) -> Tuple[float, str]:
        if not self.can_enter():
            if self.is_session_complete(): return 0.0, "SESSION_COMPLETE"
            if self.is_session_failed(): return 0.0, "SESSION_FAILED"
            if self.is_session_timeout(): return 0.0, "SESSION_TIMEOUT"
            if self.is_session_exhausted(): return 0.0, "SESSION_EXHAUSTED"
        ...
        stake = max(MIN_ORDER_AMOUNT, self._round_up_cents(raw))
    ...
    def register_win(self, amount: float, payout_pct: int) -> Tuple[float, str]:
        ...
        self.current_balance += float(amount) * payout_rate
        self.wins += 1
        self.entries += 1
        if self.is_session_complete():
            alerter.alert_session_complete(self.wins, self.current_balance or 0.0)
            return self.current_balance, "SESSION_COMPLETE"
        return self.current_balance, "WIN"

    def register_loss(self, amount: float) -> Tuple[float, str]:
        if self.current_balance is None:
            return 0.0, "NO_BALANCE"
        self.current_balance = max(0.0, self.current_balance - float(amount))
        self.losses += 1
        self.entries += 1
        if self.is_session_failed():
            alerter.alert_losing_streak(self.losses, self.current_balance or 0.0)
            return self.current_balance, "SESSION_FAILED"
        if self.is_session_exhausted():
            return self.current_balance, "SESSION_EXHAUSTED"
        return self.current_balance, "LOSS"

    def session_status(self) -> dict[str, Any]:
        elapsed_min = 0.0 if self.session_start_time is None else (time.time() - self.session_start_time) / 60.0
        return {"session_start_time": self.session_start_time, "entries": self.entries, "wins": self.wins, "losses": self.losses, "operations": self.operations, "expected_wins": self.expected_wins, "balance": self.current_balance, "initial_capital": self._initial_capital, "can_enter": self.can_enter(), "complete": self.is_session_complete(), "failed": self.is_session_failed(), "timeout": self.is_session_timeout(), "exhausted": self.is_session_exhausted(), "elapsed_min": elapsed_min, "session_max_min": self.session_max_min}
