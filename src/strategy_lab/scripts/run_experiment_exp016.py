"""Ejecuta EXP-016: brake_transition con filtro de volumen relativo alto."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_016")
...
RVOL_THRESHOLDS = [1.0, 1.5, 2.0, 3.0]


def evaluate_variant(feats: pd.DataFrame, rvol_th: float, pair: str) -> pd.DataFrame:
    mask = feats["brake_transition"].values
    rvol = feats["rvol"].values
    indices = np.where((mask) & (rvol >= rvol_th))[0]
    pair_events = []
    for idx in indices:
...
def run_single(rvol_th: float) -> dict:
    all_events = []
    for pair in PAIRS:
        df = load_m15(pair, DATA_DIR)
        feats = build_feature_frame(df)
        events = evaluate_variant(feats, rvol_th, pair)
        if len(events) > 0:
            all_events.append(events)
        print(f"rvol_th={rvol_th}: {pair} -> {len(events)} events")
    ...
    report_dir = REPORT_DIR / f"rvol_{rvol_th}"
    report_dir.mkdir(parents=True, exist_ok=True)

    artifacts = run_experiment(
        experiment_id=f"EXP-016-RVOL-{rvol_th}",
        events=events_df,
        hypothesis=f"H1-brake-rvol-{rvol_th}",
...
def main() -> None:
    summaries = []
    for rvol_th in RVOL_THRESHOLDS:
        summary = run_single(rvol_th)
        summaries.append(summary)
        print(
            f"rvol={rvol_th}: verdict={summary['verdict']}, WR={summary['win_rate']:.4f}, "
            f"EV={summary['ev']:.4f}, PF={summary['pf']:.4f}, events={summary['events_total']}"
        )

    summary_df = pd.DataFrame(summaries)
    summary_path = REPORT_DIR / "EXP-016_summary.csv"
