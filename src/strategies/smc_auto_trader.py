...
class SMCAutoTrader:
    ...
    async def run_once(self) -> RunCycleResult:
        ...
        try:
            ...
            return RunCycleResult(signal=Signal.WAIT, reason=f"error_velas:{exc}", order_placed=False)
        ...
        if len(h4) < H4_MIN_CANDLES or len(m15) < 6 or len(m1) < 6:
            ...
            return RunCycleResult(signal=Signal.WAIT, reason="datos_insuficientes", order_placed=False)
        ...
        if decision.signal == Signal.WAIT:
            return RunCycleResult(signal=Signal.WAIT, reason=decision.reason, order_placed=False)
        ...
        if self._last_trade_ts and (now - self._last_trade_ts) < self.cooldown_sec:
            ...
            return RunCycleResult(signal=decision.signal, reason=f"cooldown:{remaining}s", order_placed=False, last_trade_ts=self._last_trade_ts)

        open_asset = await self.find_open_asset()
        if not open_asset:
            log.warning("Señal %s pero no hay activo OTC abierto con payout ≥%d%%.", decision.signal.value, self.min_payout)
            return RunCycleResult(signal=decision.signal, reason="sin_activo_abierto", order_placed=False)

        trade_symbol, payout = open_asset
        direction = "call" if decision.signal == Signal.BUY else "put"
        log.info("EJECUTANDO %s  activo=%s payout=%d%%  monto=%.2f  duración=%ds", direction.upper(), trade_symbol, payout, self.amount, self.duration)

        ok, order_id, _, _, reject = await place_order(
            self.client, trade_symbol, direction, self.amount, self.duration,
            dry_run=self.dry_run, account_type=self.account_type,
        )
        if self.dry_run:
            log.info("[DRY-RUN] Orden NO enviada.")
        if not ok:
            return RunCycleResult(signal=decision.signal, reason=f"rejected:{reject}", order_placed=False, asset=trade_symbol, direction=direction)

        self._last_trade_ts = time.time()
        return RunCycleResult(signal=decision.signal, reason=decision.reason, order_placed=True, order_id=order_id, asset=trade_symbol, direction=direction, last_trade_ts=self._last_trade_ts)
