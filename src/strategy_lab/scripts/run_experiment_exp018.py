"""Ejecuta EXP-018: Solo rebote post-brake."""
...
REPORT_DIR = Path("src/strategy_lab/results/exp_018")
...
def main() -> None:
    ...
    for pair in PAIRS:
        ...
        mask = feats["rebote_mask"].values if "rebote_mask" in feats.columns else np.zeros(len(feats), dtype=bool)
        ...
        for idx in np.where(mask)[0]:
            ...
            pair_events.append({
                ...
                "body_n": float(feats.iloc[idx]["body_n"]) if "body_n" in feats.columns else np.nan,
                "brake_ratio": float(feats.iloc[idx]["brake_ratio"]) if "brake_ratio" in feats.columns else np.nan,
    ...
    if artifacts.gate_decision is None:
        raise RuntimeError("EXP-018 did not produce a gate decision")
