...
# Replay de se�ales 2026-08-10 � WIN/LOSS hipot�tico (11/08)

**Pedido TH:** cerrar el servidor, analizar la caja negra y descargar velas para saber si las se�ales del d�a hubieran sido ganadoras.

### Cierre del servidor
- Hub (app.py :8080, PID 32060) + instancia residual (:8094, PID 27396) detenidos; runtime/main.lock eliminado. Procesos hermes-agent NO tocados (orden expl�cita).
- Nota: stop_webapp.ps1 filtra .venv y NO habr�a matado esos procesos (corr�an con C:\Python314\python.exe). Cierre manual con Stop-Process.

### Caja negra (data/db/black_box_strat_2026-08-10.db) � an�lisis
...
- Las 11 BUY = tests pytest (OID-77/OID-88/TICK123; timestamps id�nticos a feature_stream). order_result="SENT" HARCODEADO en src/edificio_executor.py:447.
- Los 172 snapshots P1 reales VACIOS: direction NULL, stoch_m15={}, payout=0, candles=[] � el trazador (F41 R7) no persisti� datos reales. La �nica fuente de se�ales reales es el log.

### Replay (tools/quotex-historical-data/replay_edificio_2026-08-10.py)
- 29 se�ales extra�das del log (26 P2 + 3 P3) de ~17 activos OTC; direcci�n reconstruida con la regla EXACTA de scanner.py:1484-1502 (stoch FULL M15 + tendencia %K M1); simulaci�n 900s (entry=open vela M1 en ts; exit=close vela M1 en ts+900).
- Velas M1/M15 reales del broker (get_candles_deep, 1 d�a, copia local pyquotex + QUOTEX_DEMO_SSID + change_account PRACTICE � patr�n de src/connection.py).
- Resultado: **18 WIN / 11 LOSS (62.1%) | P&L hipot�tico +5.61 (stake 1.0)**. P2: 16/26 (61.5%) | P3: 2/3 (USCrude LOSS, BTC WIN, SOL WIN).
...
- connect() suele fallar al primer intento ("Websocket connection rejected"); con QUOTEX_DEMO_SSID en session_data + reintentos + change_account("PRACTICE") conecta (patr�n del bot).
